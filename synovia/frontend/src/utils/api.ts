...
// Get segmentation status for a case
export const getSegmentationStatus = async (caseId: string): Promise<{ status: string; message?: string }> => {
  const response = await api.get(`/segment/status/${caseId}`);
  return response.data;
};
